...
| 키         | 동작                      |
| --------- | ----------------------- |
| WASD      | 이동 (A/D = X축, W/S = Z축) |
| J         | 평타                      |
| K         | 점프                      |
| LeftShift | 대쉬                      |
| 1~4       | 동료 고유기 (라이브 페이즈)        |
| E         | 불릿타임 진입                 |
| A S D F G | 손패 카드 선택 (불릿타임 중)       |
| Space     | 콤보 실행 + 불릿타임 해제         |

기획서는 고유기를 ASDF 로 잡았지만 WASD 이동과 충돌해 숫자열로 옮겼다.
손패 ASDFG 는 불릿타임 중에만 읽으므로 이동과 겹치지 않는다.
...
`SettingsManager`, `GameManager`. 씬 · UI 의존이라 별도 작업이 필요하다.
