...
예시 두 벌은 메뉴 `Prototype > 입력 - 예시 키 프리셋 만들기` 로 만든다.

| | 기본 | 방향키 프리셋 |
| --- | --- | --- |
| 이동 · 카드 커서 · 조준 | WASD | 화살표 |
| 동료 고유기 | Z X C V | 1 2 3 4 |
| 평타 · 점프 · 대쉬 | J K LShift | Z X C |
| 불릿타임 | E | Space |
| 카드 즉시 사용 | U | A |
| 조준 확정 · 취소 | J K | Z X |
...
조준 확정의 마우스 클릭은 프리셋이 안 건드린다. `part` 가 빈 항목은 키보드 자리(0번)만 바꾼다.
