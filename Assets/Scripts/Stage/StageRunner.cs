...
namespace Prototype
{
    ...
    public class StageRunner : MonoBehaviour
    {
        ...
        /// <summary>
        /// <b>진행 판정은 여기서 안 한다.</b> 조우를 도는 것은 <see cref="StageDirector"/>고,
        /// 승패 판정은 그쪽 하나만 물어본다. 이 컴포넌트에 남은 일은 구간과 카메라뿐이다.
        /// </summary>
        ...
        private void Awake()
        {
            ...
            if (director == null) director = FindAnyObjectByType<StageDirector>();
        }
        ...
        private void Start()
        {
            ...
            Debug.Log($"[StageRunner] 구간 {sections.Length}개 — {sections[start].kind}에서 시작");
        }
        ...
        private void Update()
        {
            ...
            // 조우가 도는 동안에는 위치를 안 본다. 넉백으로 경계에 걸치는 한 프레임에
            // 카메라 락이 풀리면 전투 도중 화면이 통째로 흔들린다.
            EncounterSite busy = director != null ? director.ActiveSite : null;
            if (busy != null)
            {
                bounds?.SwitchTo(busy.MinX, busy.MaxX);
                return;
            }
        ...
        /// 구간의 경계를 카메라에 넘긴다.
        ///
        /// <b>조우를 여는 일은 더 이상 여기서 안 한다.</b> 진입선 판정은 조우가 들고 있고
        /// (<see cref="EncounterTrigger.CrossLine"/>) 디렉터가 본다 — 구간과 조우를
        /// 왼쪽 경계로 짝지어 찾던 옛 방식은 둘이 어긋나면 조용히 적이 안 나왔다.
        /// </summary>
        ...
        public void Configure(StageSection[] stageSections, StageBounds owner)
        {
